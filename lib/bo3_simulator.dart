...
class BO3SimulatorPage extends StatefulWidget {
  const BO3SimulatorPage({super.key});

  @override
  State<BO3SimulatorPage> createState() => _BO3SimulatorPageState();
}

class _BO3SimulatorPageState extends State<BO3SimulatorPage> {
  ...
  String selectedDrawMode = "none";
  double customDrawRate = 0.10;

  final Map<String, double> drawRates = {"none": 0.0, "auto": 0.01};
  ...
  void simulateBO3Tournament() {
    final rand = Random();
    Map<int, int> occurrences = {};
    Map<int, int> passes = {};
    Map<int, int> cutoffCounts = {};

    double drawRate = selectedDrawMode == "custom"
        ? customDrawRate
        : drawRates[selectedDrawMode] ?? 0.0;
  ...
  String interpretScore(double ratio, int count, String drawMode) {
    ...
    if (ratio < 0.95) {
      return drawMode == "none"
          ? "🟢 Ça passe presque toujours. Ne joue pas la montre."
          : "🟢 Ça passe presque toujours. Un ID peut suffire.";
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Simulation Complète BO3')),
      body: Padding(
        ...
        child: ListView(
          children: [
            ...
            Slider(
              ...
              divisions: 9,
              label: "$numRounds",
              onChanged: (val) => setState(() => numRounds = val.toInt()),
            ),
            ...
            DropdownButton<int>(
              ...
              items: [0, 4, 8, 16, 32, 64, 128]
                  ...
                  .toList(),
              onChanged: (val) => setState(() => topCut = val!),
            ),
            const SizedBox(height: 12),
            const Text("Probabilité de Draw (match nul) :"),
            DropdownButton<String>(
              value: selectedDrawMode,
              items: [
                DropdownMenuItem(value: "none", child: Text("aucun draw")),
                DropdownMenuItem(
                  value: "auto",
            ...
            if (selectedDrawMode == "custom") ...[
              Text(
                "Draw personnalisé : ${(customDrawRate * 100).toStringAsFixed(1)}%",
              ),
              Slider(
                ...
                max: 0.30,
                divisions: 28,
                label: "${(customDrawRate * 100).toStringAsFixed(1)}%",
                onChanged: (val) => setState(() => customDrawRate = val),
              ),
            ],
            ...
            if (scoreOccurrences.isNotEmpty) ...[
              ...
              ...(() {
                ...
                return displayed.map((score) {
                  ...
                  final label = interpretScore(ratio, freq, selectedDrawMode);
                  final percentage =
                      ((freq /
              ...
              if (!showAllResults)
                TextButton(
                  onPressed: () => setState(() => showAllResults = true),
